...
VPC flow logs are enabled by default. As of the time of this writing, all traffic is sent to the Log Archive account. To change where the logs are sent, edit variable `global_flow_log_destination_arn` the value in `locals.tf` to point to the correct bucket ARN.
...
| [aws_flow_log.global_flow_log](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/flow_log) | resource |
...
| <a name="input_flow_log_per_hour_partition"></a> [flow\_log\_per\_hour\_partition](#input\_flow\_log\_per\_hour\_partition) | (Optional) Indicates whether to partition the flow log per hour. This reduces the cost and response time for queries. | `bool` | `true` | no |
...
| <a name="input_nat_eip_tags"></a> [nat\_eip\_tags](#input\_nat\_eip\_tags) | Additional tags for the NAT EIP | `map(string)` | <pre>{<br>  "type": "NAT"<br>}</pre> | no |
